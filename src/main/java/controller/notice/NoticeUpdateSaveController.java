...
public class NoticeUpdateSaveController implements Controller {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String attachfile = multiRequest.getOriginalFileName("att_file");
		//서버로 업로드된 파일의 파일명
		String filename = multiRequest.getFilesystemName("att_file");
		logger.info("::: 파일명-{} from {}  :::",filename,attachfile);   //uploadFile
		//서버의 파일 시스템 C:\\upload 경로에서 파일이 생겼는지 확인하기
		
		
		String temp=multiRequest.getParameter("notice_idx");
		int notice_idx = Integer.parseInt(temp);
		String title=multiRequest.getParameter("title");
		String content=multiRequest.getParameter("contents");
		
		String uploadFile = multiRequest.getParameter("up_file");
		if(uploadFile !=null) {		//첨부된 파일을 수정하지 않은 경우
			filename=uploadFile;
			attachfile=multiRequest.getParameter("att_file");
		}
		...
		if(result!=0) {
			String url = "read?notice_idx="+notice_idx;
			/*
